...
angular
  ...
  .service(
     ...
     */
      ...
      //-------------------------------------------------------------------
      //-- CLASS: Form for choosing the custom (user) boards directory
      //-- (profile 'externalBoards'). Boards placed inside it, with the
      //-- same format as the distribution boards, are always scanned and
      //-- offered in every board list.
      //-------------------------------------------------------------------
      class FormExternalBoards extends Form {
        constructor(boardsPath) {
          //-- Create a blank Form (calling the upper Class)
          super();

          //----- Add the different Fields:

          //-- Field 0: Custom boards path (directory chooser)
          let field0 = new DirectoryField(
            gettextCatalog.getString(
              'Select a directory for your custom boards. ' +
                'Icestudio will always scan this folder and offer any board ' +
                'you place or create inside it (same format as the ' +
                'distribution boards) in every board list, without copying ' +
                'it into each project. This folder is required and must exist.'
            ), //-- Top message
            boardsPath, //-- Current/proposed directory
            0, //-- Field id
            gettextCatalog.getString('No directory selected') //-- Placeholder
          );
      ...
      this.FormExternalBoards = FormExternalBoards;
    }
  );
